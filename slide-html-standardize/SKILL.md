...
description: >
  Normalizes AI-generated HTML slides before conversion — ensures consistent
  viewport, .slide wrapper, strips animations and external resources. Use this
  skill whenever working with HTML slides generated by ChatGPT, Claude, Gemini,
  or any AI that produces HTML slide decks. Also use when Chrome extraction fails
  or produces empty results — the HTML may be missing a .slide wrapper or have a
  wrong viewport. If the user says "these slides came from AI" or "the HTML
  doesn't convert properly" or "extraction returns nothing", run standardization
  first. This is always the correct first step before slide-html-to-pptx when
  dealing with AI-generated HTML that hasn't been preprocessed.
---
...
```bash
python slide-html-standardize/scripts/html_standardize.py slide.html
```

Normalize a single file and write the result to a new path:

```bash
python slide-html-standardize/scripts/html_standardize.py slide.html -o slide_clean.html
```

Normalize every `.html` file in a directory (each file is overwritten in place):

```bash
python slide-html-standardize/scripts/html_standardize.py --dir ./slides/
```

Normalize with a custom viewport size (default is 1280x720):

```bash
python slide-html-standardize/scripts/html_standardize.py slide.html --width 1920 --height 1080
```

Combine directory mode with a custom viewport:

```bash
python slide-html-standardize/scripts/html_standardize.py --dir ./slides/ --width 1920 --height 1080
```
